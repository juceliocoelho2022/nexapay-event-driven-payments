...
$results = [ordered]@{
    ...
    "Frontend dependencies" = $null
    "Frontend production build" = $null
...
function Start-ServiceIfNeeded {
    ...
    $stdout = Join-Path $env:TEMP ("nexapay-{0}-sprint9-{1}.out.log" -f $Label.ToLower(), $runId)
    $stderr = Join-Path $env:TEMP ("nexapay-{0}-sprint9-{1}.err.log" -f $Label.ToLower(), $runId)
...
function Start-FrontendIfNeeded {
    ...
    $stdout = Join-Path $env:TEMP ("nexapay-frontend-sprint9-{0}.out.log" -f $runId)
    $stderr = Join-Path $env:TEMP ("nexapay-frontend-sprint9-{0}.err.log" -f $runId)
    ...
    Write-Host "[Frontend] started by validation script. PID=$($process.Id)"
    return $process
}
...
try {
    ...
    Invoke-Check "Full Maven reactor" {
        Invoke-NativeChecked -FilePath (Get-Command mvn).Source -Arguments @("-B", "clean", "test") -FailureMessage "Maven reactor failed."
    }

    Invoke-Check "Frontend dependencies" {
        Push-Location $frontendRoot
        try {
            Invoke-NativeChecked -FilePath (Get-Command npm).Source -Arguments @("install", "--package-lock=false", "--no-audit", "--no-fund") -FailureMessage "npm install failed."
        }
        finally { Pop-Location }
    }

    if ($results["Frontend dependencies"] -ne $true) {
        throw "Frontend dependency installation failed; remaining frontend validation was skipped."
    }

    Invoke-Check "Frontend production build" {
        Push-Location $frontendRoot
        try {
            Invoke-NativeChecked -FilePath (Get-Command npm).Source -Arguments @("run", "build") -FailureMessage "Frontend build failed."
            $indexPath = Join-Path $frontendRoot "dist\index.html"
            if (-not (Test-Path $indexPath)) { throw "dist/index.html was not generated." }
        }
        finally { Pop-Location }
    }

    if ($results["Full Maven reactor"] -ne $true -or $results["Frontend production build"] -ne $true) {
        throw "Build validation failed; runtime Sprint 9 checks were skipped."
    }

    Write-Host "`n=== Starting NexaPay infrastructure for Sprint 9 ==="
    ...
    if ($null -ne $frontendProcess) { $ownedProcesses += $frontendProcess }
    ...
    if ($results["Register/login through frontend"] -ne $true) {
        throw "Auth flow failed; protected frontend API checks were skipped."
    }
    ...
    Invoke-Check "PIX create/read through frontend" {
        ...
        $paymentBody = @{
            ...
            amount = 123.45
            description = "Sprint 9 frontend validation"
        } | ConvertTo-Json -Compress

        $headers = @{
            Authorization = "Bearer $token"
            "Idempotency-Key" = "sprint9-$runId"
        }
...
Write-Host "`n========================================"
Write-Host "       NEXAPAY SPRINT 9 VALIDATION"
Write-Host "========================================"
...
if ($null -ne $fatalError -or $failed.Count -gt 0) {
    exit 1
}
exit 0
